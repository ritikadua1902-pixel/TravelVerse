...
import {
  MapPin, Trash2, Plus, Sparkles, AlertTriangle, X, Image as ImageIcon,
} from 'lucide-react';
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="border-t border-white/[0.06] pt-5">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-sm font-semibold text-red-400 flex items-center gap-2"><AlertTriangle size={16} /> Red Zones</h3>
                      <button type="button" onClick={() => addSubItem('redZone')}
                        className="bg-red-500/10 hover:bg-red-500/20 border border-red-500/15 px-3 py-1.5 rounded-lg text-xs font-medium text-red-400 transition-colors">
                        + Add Zone
                      </button>
                    </div>
                    <div className="space-y-3">
                      {redZones.map((rz) => (
                        <div key={rz.id} className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3 items-center">
                          <input type="text" placeholder="Name" className={inputCls} value={rz.name} onChange={(e) => updateSubItem('redZone', rz.id, 'name', e.target.value)} />
                          <input type="text" placeholder="Description" className={inputCls} value={rz.description} onChange={(e) => updateSubItem('redZone', rz.id, 'description', e.target.value)} />
                          <input type="number" step="any" placeholder="Lat" className={inputCls} value={rz.coordinates[0]} onChange={(e) => updateSubItemCoord('redZone', rz.id, 0, e.target.value)} />
                          <input type="number" step="any" placeholder="Lng" className={inputCls} value={rz.coordinates[1]} onChange={(e) => updateSubItemCoord('redZone', rz.id, 1, e.target.value)} />
                          <button type="button" onClick={() => removeSubItem('redZone', rz.id)} className="text-red-400/60 hover:text-red-400 transition-colors justify-self-start xl:justify-self-center"><X size={16} /></button>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="border-t border-white/[0.06] pt-5">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-sm font-semibold text-amber-400 flex items-center gap-2"><Sparkles size={16} /> Hidden Gems</h3>
                      <button type="button" onClick={() => addSubItem('hiddenGem')}
                        className="bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/15 px-3 py-1.5 rounded-lg text-xs font-medium text-amber-400 transition-colors">
                        + Add Gem
                      </button>
                    </div>
                    <div className="space-y-3">
                      {hiddenGems.map((hg) => (
                        <div key={hg.id} className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3 items-center">
                          <input type="text" placeholder="Name" className={inputCls} value={hg.name} onChange={(e) => updateSubItem('hiddenGem', hg.id, 'name', e.target.value)} />
                          <input type="text" placeholder="Description" className={inputCls} value={hg.description} onChange={(e) => updateSubItem('hiddenGem', hg.id, 'description', e.target.value)} />
                          <input type="number" step="any" placeholder="Lat" className={inputCls} value={hg.coordinates[0]} onChange={(e) => updateSubItemCoord('hiddenGem', hg.id, 0, e.target.value)} />
                          <input type="number" step="any" placeholder="Lng" className={inputCls} value={hg.coordinates[1]} onChange={(e) => updateSubItemCoord('hiddenGem', hg.id, 1, e.target.value)} />
                          <button type="button" onClick={() => removeSubItem('hiddenGem', hg.id)} className="text-amber-400/60 hover:text-amber-400 transition-colors justify-self-start xl:justify-self-center"><X size={16} /></button>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <motion.div key={place._id}
                ...
                <div className="p-6">
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    ...
                    className="w-full bg-red-500/10 hover:bg-red-500/15 border border-red-500/15 py-2.5 rounded-xl flex items-center justify-center gap-2 text-red-400 text-xs font-medium transition-colors">
